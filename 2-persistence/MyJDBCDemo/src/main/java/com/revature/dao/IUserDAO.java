...
public interface IUserDAO {
	...
	//Here we outline all of the methods that the DAO Implmentation class must implement to perform
	//the following CRUD methods on User objects only
	
	//Insert a User object, and return the primary key that DB generates
	int insert(User u);
	...
	//Update:
	boolean update(User u); //update that object in the DB and return true if it was updated sucessfully or false if there was a problem
	
	//Delete:
	boolean delete(int id); // tak in the id of the User we want to delete in the DB
