 ...
 */
@WebServlet("/HelperSessionServlet")
public class HelperSessionServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (vill != null) {
			out.println("we have capture the villain from the session");
			
			out.println("<h1>Villain name: " + vill.getName() + "</h1><br />");
			out.println("<b>Villain name: " + vill.getSuperPower() + "</b><br />");
			out.println("<i>Villain name: " + vill.getBounty() + "</i><br />");
		} else {
