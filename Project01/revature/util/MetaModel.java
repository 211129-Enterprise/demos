...
/**
 * This class' job is to gather as much information as possible about the class
 * we want to transpose into a DB Entity (table).
 * 
 * This class's job is to model data about another class.
 */
public class MetaModel<T> { // we're inferring that the MetaModel class can only be a metamodel of another
							// class
	...
	public ColumnField getColumn(String columnName) {
		Optional<ColumnField> maybe = getColumns().stream().filter(m -> m.getColumnName().equals(columnName))
				.findFirst();
	...
	//		throw new RuntimeException("No foreign keys found in: " + clazz.getName());
			...
			return new LinkedList<ForeignKeyField>();
		}
